...
Flux-Titan is designed for people who need a practical, low-maintenance news pipeline:

- **Small Telegram channels** that need regular updates without manual posting.
- **Niche communities** (AI, crypto, devtools, local industries) that want curated digests from specific RSS sources.
- Builders running **self-hosted news digests** on their own infra (or GitHub Actions).
- Teams that want **reusable automation** they can adapt and keep simple.

---

## 🧾 Example output

A typical generated Telegram post looks like:

```html
🔶 <b>OpenAI launches a faster inference tier</b>

Компания представила новый режим обработки запросов с меньшей задержкой и более предсказуемой стоимостью. По первым данным, это помогает быстрее отдавать ответы в нагруженных сценариях и снижать операционные издержки для команд с большим трафиком.

Для каналов и продуктовых команд это означает более дешёвую и стабильную автоматизацию контентных задач.

#AI #Automation #Tech
🔗 <a href="https://example.com/article">Read full article</a>
```

> Output is generated by your selected model (Gemini or OpenAI), but kept in a Telegram-friendly HTML format.

### 📸 Result screenshot / GIF (placeholder)

> **No real screenshot is currently committed in this repository.**
>
> Add an image at `docs/assets/telegram-result.png` (or GIF at `docs/assets/telegram-result.gif`) and link it here.
>
> The screenshot/GIF should show:
> 1. A real post published by Flux-Titan in Telegram.
> 2. Headline + summary + hashtags + source link in one message.
> 3. (Optional) image attachment if `og:image` extraction succeeds.

```md
![Flux-Titan Telegram output](docs/assets/telegram-result.png)
```
...
*   **Retry/Backoff for transient API issues:** Automatically retries temporary AI provider and Telegram API failures with bounded exponential backoff.
...
2.  Prepare your local config files:
    ```bash
    cp .env.example .env
    cp examples/feeds.yaml ./feeds.yaml
    ```
3.  Fill `.env` with your real credentials (`TG_TOKEN`, `CHANNEL_ID`, and provider API key).
4.  Run one pipeline pass:
    ```bash
    docker compose up --build
    ```

    *(The bot runs once and exits by design.)*
...
docker run --env-file .env \
  -v $(pwd)/processed.db:/app/processed.db \
  -v $(pwd)/feeds.yaml:/app/feeds.yaml \
  flux-titan
```
...
### 1) Small Telegram channel (one-person editorial workflow)
Use Flux-Titan to auto-publish short, consistent updates a few times per day without manual copy-paste.

Typical setup:
- 3-5 trusted RSS sources.
- `MAX_ARTICLES_PER_RUN=3..5` to avoid flood.
- Scheduled runs (hourly) via GitHub Actions or host cron.

### 2) Niche news digest (focused community)
Use Flux-Titan to aggregate and summarize highly specific sources (for example: AI tooling, cybersecurity, regional tech ecosystem) into one Telegram digest stream.

Typical setup:
- Curated YAML feeds list with source icons.
- Consistent output style from one selected AI provider.
- SQLite deduplication to avoid reposting the same links.

### Minimal 3-5 feed configuration example
...
For a practical first production run (small channel or niche digest):

1. Create runtime config files:
   ```bash
   cp .env.example .env
   cp examples/feeds.yaml ./feeds.yaml
   ```
2. Edit `.env` with real credentials:
   - `TG_TOKEN`
   - `CHANNEL_ID`
   - `AI_PROVIDER` + matching API key
3. Keep initial limits conservative:
   - `MAX_ARTICLES_PER_RUN=3` (recommended starting point)
4. Run once and verify output in Telegram:
   ```bash
   flux-titan
   ```
5. Add scheduling:
   - GitHub Actions (hourly) **or** host cron.

This keeps deployment simple and matches the current one-shot pipeline design.
