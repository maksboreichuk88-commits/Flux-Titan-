...
async def test_openai_summarizer_integration(article):
    with patch("flux_titan.summarizers.openai.OpenAI") as mock_openai_cls:
        ...
        assert article["link"] in result
        assert summarizer.base_url == "https://example.com/v1"


@pytest.mark.asyncio
async def test_kimi_summarizer_integration_uses_openai_compatible_path(article):
    with patch("flux_titan.summarizers.openai.OpenAI") as mock_openai_cls:
        mock_client = MagicMock()
        mock_openai_cls.return_value = mock_client
        ...
        mock_completion = MagicMock()
        mock_completion.choices = [
            MagicMock(message=MagicMock(content="<b>Kimi Insights</b>\n\nAI moves fast.\n\n#Kimi\n<a href='https://example.com/ai'>Source</a>"))
        ]
        mock_client.chat.completions.create.return_value = mock_completion

        summarizer = KimiSummarizer(api_key="fake_key", model="kimi-k2.5")
        result = await summarizer.summarize(article)

        assert summarizer.model_name == "moonshotai/kimi-k2.5"
        assert summarizer.base_url == "https://integrate.api.nvidia.com/v1"
        assert result is not None
        assert "Kimi Insights" in result
        assert "#Kimi" in result
        assert article["link"] in result
        assert mock_client.chat.completions.create.call_args.kwargs["model"] == "moonshotai/kimi-k2.5"
